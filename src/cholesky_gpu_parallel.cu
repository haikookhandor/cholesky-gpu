...
// Kernel to perform the pivot step: compute square root of diagonal element A[k][k]
__global__ void cholesky_pivot(double **elements, int n, int k) {
    // Ensure only a single thread performs this operation
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        elements[k][k] = sqrt(elements[k][k]);
    }
}

// Kernel to normalize row k: divide each element in row k (after the diagonal) by A[k][k]
__global__ void cholesky_normalize(double **elements, int n, int k) {
    int col = k + 1 + blockIdx.x * blockDim.x + threadIdx.x;
    if (col < n) {
        elements[k][col] /= elements[k][k];
    }
}

// Kernel to update the trailing submatrix after elimination step
__global__ void cholesky_update(double **elements, int n, int k) {
    int row = k + 1 + blockIdx.y * blockDim.y + threadIdx.y;
    int col = k + 1 + blockIdx.x * blockDim.x + threadIdx.x;
    if (row < n && col < n && col >= row) {
        elements[row][col] -= elements[k][row] * elements[k][col];
    }
}

// Kernel to zero out the lower triangular elements below the diagonal in column k
__global__ void cholesky_lower_zero(double **elements, int n, int k) {
...
int main(int argc, char *argv[]) {
    ...
    // Read input, validate
    if (argc != 5) {
        printf("Usage: <executable_name> <matrix_size> <tpb> <block_x> <block_y>\n");
        exit(0);
    }
    ...
    for (int k = 0; k < dA.n; ++k) {
        // Step 1: Pivot - compute sqrt of diagonal element at (k, k)
        cholesky_pivot<<<1, 1>>>(dA.elements, dA.n, k);
        err = cudaGetLastError(); check_error(err, ERR_KERNEL);
        cudaDeviceSynchronize();

        // Step 2: Normalize row k (divide each element in row k beyond diagonal by A[k][k])
        int remaining_cols = dA.n - (k + 1);
        if (remaining_cols > 0) {
            int grid_x = (remaining_cols + tpb - 1) / tpb;  // 1D grid
            cholesky_normalize<<<grid_x, tpb>>>(dA.elements, dA.n, k);
            err = cudaGetLastError(); check_error(err, ERR_KERNEL);
            cudaDeviceSynchronize();
        }

        // Step 3: Update trailing submatrix (Schur complement)
        int rem_size = dA.n - (k + 1);
        if (rem_size > 0) {
            dim3 block_dim(block_x, block_y);
            dim3 grid_dim((rem_size + block_x - 1) / block_x,
                        (rem_size + block_y - 1) / block_y);
            cholesky_update<<<grid_dim, block_dim>>>(dA.elements, dA.n, k);
            err = cudaGetLastError(); check_error(err, ERR_KERNEL);
            cudaDeviceSynchronize();
        }

        // Step 4: Zero the lower part of column k (for upper triangular result)
        if (rem_size > 0) {
            int grid_x = (rem_size + tpb - 1) / tpb;
            cholesky_lower_zero<<<grid_x, tpb>>>(dA.elements, dA.n, k);
            err = cudaGetLastError(); check_error(err, ERR_KERNEL);
            cudaDeviceSynchronize();
        }
    }
